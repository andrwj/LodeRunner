...
void setup()
{
  ...
  if (gameNumber < GAME_NUMBER)
    gameState = initialStates[gameNumber];
...
void GameSelect()
{
  ...
  if (!firstTime)
  {
    if ((buttons & UP_BUTTON) && menuSelect > 0)
    {
      menuSelect--;
    }
    if ((buttons & DOWN_BUTTON) && menuSelect < 1)
    {
      menuSelect++;
    }
  }
...
void playerDies()
{
  ...
  gameState = GameState::LevelExitInit;
  player.setNextState((menLeft > 0) ? GameState::RestartLevel : GameState::GameOver);
  // if (menLeft > 0)
  // {
